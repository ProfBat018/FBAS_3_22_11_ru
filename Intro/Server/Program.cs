...
var ipAddr = IPAddress.Parse("172.20.208.6");
var ipEndPoint = new IPEndPoint(ipAddr, 11000);

// AdressFamily.InterNetwork - это IpV4
// Так как мы работаем с помощью протокола Tcp, то нам нужен тип сокета - Stream 
...
try
{
    ...
    while (true)
    {
        Console.WriteLine($"Waiting for a connection from adress {ipAddr}");
